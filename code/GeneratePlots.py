    ...
    if args.type == 'SEEDSvsUTILITY':
            ...
            if "backup" not in root:
                ...
                for est in y_dict:
                    if est != ('polynomial', 1):
                        ...
                        # # FW_iterations = []
                        # objectives = []  # F(y) where F is the multilinear relaxation or F^(y) where F^ is the best
                        # estimator
                        # if file_name[0] == 'polynomial':
                        #     poly_dict[track[max_key][0]] = problem.utility_function(track[max_key][1])
                        #     poly_label = 'POLY'
        ...
        for root in files: # keys of the files dict, keys are directory names
            if "backup" not in root:
                ...
                # plt.title("Selection of a subset of k = " + str(title_str[1]) + " with " + str(title_str[2])
                # + " FW iterations")
